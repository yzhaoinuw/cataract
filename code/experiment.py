...
class Experiment:
    def __init__(self, X, y, test_size=0.4, split_seed=None, h1=128, dropout=0.5):
    ...
    def _initialize(self, X, y):
        X_train, X_test, y_train, y_test = train_test_split(
            X, y, test_size=self.test_size, random_state=self.split_seed
        )
        ...
        # Initialize the MLP
        self.mlp = MLP(input_size=self.input_size, h1=self.h1, dropout=self.dropout).to(
            self.device
        )
